...
$(document).ready(function(){
    ...
    $(".deleteCampaignButton").click(function () {
        ...
        let result =  $(".deleteCampaignButton").attr("href") +idArr.join("-");
        console.log(result);
        $(".deleteCampaignLink").attr("href",result);
        $("#deleteCampaign .modal-body").text("Bạn có muốn xóa "+idArr.length+" đợt quyên góp?")

    })
    $(".deleteCampaignModalOk").click(function (e) {
    ...
    $(".deleteOrganizationButton").click(function () {
        let idArr = [];
        $.each($("input.ds:checked"), function () {
            idArr.push($(this).val());
        });
        $(this).attr("id",idArr.join("-"));
        let result =  $(".deleteOrganizationButton").attr("href") +idArr.join("-");
        console.log(result);
        $(".deleteOrganizationLink").attr("href",result);
        $("#deleteOrganization .modal-body").text("Bạn có muốn xóa "+idArr.length+" đơn vị tổ chức?")
    ...
    $(".editCampaignLink")?.click(function (e) {
        ...
        if ($("input.ds:checked").length > 1){
            e.preventDefault();
            $("#editCampaignModal").modal('show');
        }
        $(this).attr("href","/management/campaign/edit/"+$("input.ds:checked").val());
    })
    ...
    $(".editOrganizationLink")?.click(function (e) {
        ...
        $(this).attr("href","/management/organization/edit/"+$("input.ds:checked").val());
    })
    // disable form submissions if there are invalid fields for login form
    $(".login").submit(function (e) {
/*        const mailFormat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;

        let email = $(".login #email");
        if(!mailFormat.test(email.value)){
            email[0]?.setCustomValidity("The format of email is wrong");
        }*/
